...
![포트폴리오 히트맵](images/user_guide_v2_0/control_portfolio_heatmap.png)

- 왼쪽 시각화 영역은 `비중`과 `히트맵`을 전환해 볼 수 있다.
- 히트맵에서 박스 크기는 현재 평가액 비중, 색상은 선택 기간 수익률을 뜻한다.
- 종합 히트맵에서 종목을 누르면 해당 포트폴리오 상세로 이동하고, `종합으로` 버튼으로 돌아올 수 있다.
...
- 백업 파일에는 포트폴리오, 보유종목, 매매기록, 배당기록, 사용자 표시명, 설정이 포함된다.
