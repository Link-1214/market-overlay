...
현재 포함된 이미지는 v2.0 계열 화면 캡처를 재사용한 임시 예시다. v2.2의 실제 최신 캡처로 교체되지는 않았으므로, v2.2에서 추가된 진단 로그, state schema, 포트폴리오 시각화 영역 조절은 아래 문구로 보강한다. Yahoo 전일 종가 보정은 v2.1에서 이미 추가된 정책이고, v2.2에서는 이 보정 경로를 market data provider와 회귀 검증 안으로 정리했다. 기존 `히트맵` 용어는 `트리맵`으로 정리했다.
...
- Yahoo 일봉에서 전일 종가가 잠시 비면 5일 60분봉으로 보정해 전일대비가 비정상적으로 튀는 상황을 줄인다. 이 보정은 v2.1부터 적용된 정책이며, v2.2에서는 provider 분리와 회귀 검증으로 유지된다.
