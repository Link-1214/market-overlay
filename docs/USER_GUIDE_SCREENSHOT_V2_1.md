# Market Overlay 화면 사용 가이드 v2.1

이 문서는 GitHub 공개 가이드에 넣기 위한 스크린샷 중심 사용 설명이다. 아래 화면은 실제 종목을 사용한 예시 데이터이며, 투자 판단을 대신하지 않는다.

스크린샷은 v2.0과 공통인 화면 구조를 사용한다. v2.1에서는 `히트맵` 명칭이 `트리맵`으로 바뀌었고 설정 탭에 업데이트 확인 영역이 추가됐다.
...
- v2.1부터 현재 버전, 자동 업데이트 확인 여부, 수동 확인, 숨긴 버전 다시 알림을 설정 탭에서 관리한다.
...
- 왼쪽 시각화 영역은 `비중`과 `트리맵`을 전환해 볼 수 있다.
...
- 백업 파일에는 포트폴리오, 보유종목, 매매기록, 배당기록, 사용자 표시명, 업데이트 확인을 포함한 앱 설정이 들어간다.
